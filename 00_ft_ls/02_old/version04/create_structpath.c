...
/*   Updated: 2017/01/08 14:54:52 by curquiza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_list		*ft_get_lst_in(char *filename, int *permerror, char *opt)
{
	DIR				*dirp;
...
t_path		*ft_new_structpath(char *filename, char *path, char *opt)
{
	...
	new->inside = NULL;
	new->permerror = 0;
	if (lstat(path, &(new->lstat)) >= 0)
	{
		new->display = ft_strdup(filename);
		new->lst_in = ft_get_lst_in(path, &new->permerror, opt);
	}
	new->error = errno;
...
t_path		**ft_new_inside(t_path *path, char *opt)
{
	...
	while (i < cpt)
	{
		tmppath = ft_strjoin3(path->name, "/", current->content);
		inside[i] = ft_new_structpath(current->content, tmppath, opt);
		ft_strdel(&tmppath);
		i++;
