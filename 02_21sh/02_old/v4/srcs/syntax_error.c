...
/*   Updated: 2017/05/13 18:09:42 by curquiza         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
** tenant pas compte des éventuels tokens de type NEWLINE)
*/
...
char	*ft_check_tokenerror(t_lexeme *lex)
{
	...
	while (lex)
	{
		if (lex->op == DSEMIC || lex->op == DPIPE || lex->op == AND
			|| lex->op == DAND || lex->op == LESSGREAT || lex->op == DLESSDASH
			|| lex->op == CLOBBER)
			return (lex->s);
